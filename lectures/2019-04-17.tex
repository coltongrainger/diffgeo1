...
\begin{defn}[Coframe fields]
    ...
    Suppose $(E^1, \ldots, E^n)$ is a frame field on $U \subset M$, then the \term{dual coframe field} $(\epsilon^1, \ldots, \epsilon^n)$ is defined by (the Krönecker deltas) condition
    \[
...
\begin{defn}[Differential of a function]
   Let $M \in \Man$ and $f \in C^\infty(M)$. The \term{differential of $f$} is the covector field $\dd f$ on $M$ defined by, for all $\vec v \in T_p M$,
   \[
